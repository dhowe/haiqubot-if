...
/* router.get('/app/:conn/:db/:coll/select/:doc_id', function (req, res, next) {
    
    console.log('\nSELECT\n');
    
    var connection_list = req.app.locals.dbConnections;
    
    //req.params.doc_id = req.params.doc_id.length > 10 ? req.params.doc_id : '5e86ba3b42ea447e402fac53';
    ...
    common.get_sidebar_list(mongo_db, req.params.db, function (err, sidebar_list) {
        common.get_id_type(mongo_db, req.params.coll, req.params.doc_id, function (err, result) {
            ...
            res.render('coll-edit', {
                ...
                sidebar_list: sidebar_list,
                coll_name: req.params.coll,
                coll_doc: stringifyTweet(result.doc),
                helpers: req.handlebars.helpers,
                editor: true,
...
function stringifyTweet(tweet) {
    let { selected, haiku } = tweet;
    let keys = Object.keys(tweet);
    let first = ['haiku', 'selected'];
    const fkeys = keys.filter(k => !first.includes(k));
    console.log('keycount', keys.length, fkeys.length);
    fkeys.unshift(...first);
    return bsonify.stringify(tweet, fkeys, '    ');
}
    ...
    common.get_sidebar_list(mongo_db, req.params.db, function (err, sidebar_list) {
        common.get_id_type(mongo_db, req.params.coll, req.params.doc_id, function (err, result) {
            ...
            res.render('coll-edit', {
                ...
                //coll_doc: stringifyTweet(result.doc, null, '    '),
                helpers: req.handlebars.helpers,
                editor: true,
                images_fields: images,
                video_fields: videos,
